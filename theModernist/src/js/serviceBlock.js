function ServiceBlock() {
   const service_blocks = document.getElementsByClassName('serviceBlock__item');
   service_blocks[0].classList.add('serviceBlock__item-click');
   const par_service_blocks = document.querySelector('.main-serviceBlock');
   par_service_blocks.addEventListener(`click`, setLatestWorkContent);
   function setLatestWorkContent(e) {
      for (let i = 0; i < service_blocks.length; i++) {
         if (e.target.classList.contains('main-serviceBlock')) {
            return
         }
         if (e.target.classList.contains('serviceBlock__item-click')) {
            e.target.classList.remove('serviceBlock__item-click');
            break
         };
      };
